...
namespace cms {
	...
	class Graph {
	  public:
		...
		unsigned road_network_length = 0;		
		unsigned rnl_not_covered = 0;
		unsigned rnl_covered_by_one_unit = 0;
		unsigned rnl_covered_by_more_than_one_unit = 0;									
	    ...
	    void load_from_binary(std::string filename)
	    {
			...
			// à déplacer où le graphe est construit
			for (unsigned i = 0; i < this->rk_graph.geo_distance.size(); ++i)
			{
				this->road_network_length += this->rk_graph.geo_distance[i];
			}
			this->rnl_not_covered = this->road_network_length;
	    }	
	    ...
	    void export_graph_properties_to_csv(std::string destination_folder)
	    {
			...
			/*** Other property ***/
			//export_routing_graph_element(destination_folder + "osmwayid_to_idx.csv", this->osmwayid_to_idx);
		}
		...
		std::vector<std::pair<double,double>> get_nodes_from_way_into_vec(uint64_t way_idx)
		{
	        std::vector<std::pair<double,double>> way_nodes;
	        for(size_t i = 0; i < opr_graph.ways[way_idx].size(); i++){
				way_nodes.push_back( std::make_pair( opr_graph.nodes[opr_graph.ways[way_idx][i]].lat_m, opr_graph.nodes[opr_graph.ways[way_idx][i]].lon_m ) );
	        }

	        return way_nodes;
		}

		double get_geo_distance_from_way(uint64_t way_idx)
		{

	        float distance = 0;
	        for(size_t i = 0; i < opr_graph.ways[way_idx].size() - 1; i++){
				distance += coordinates_to_distance( \
					opr_graph.nodes[opr_graph.ways[way_idx][i]].lat_m, \
					opr_graph.nodes[opr_graph.ways[way_idx][i]].lon_m, \
					opr_graph.nodes[opr_graph.ways[way_idx][i+1]].lat_m, \
					opr_graph.nodes[opr_graph.ways[way_idx][i+1]].lon_m \
				);
	        }
	        return distance;
		}
	...
	class GraphCH : public Graph
	{
	  public:
		...
		// Building coverage mapping: segment OSM ID -> list of building IDs
		std::unordered_map<uint64_t, std::vector<uint64_t>> segment_to_buildings;
		bool building_mapping_loaded = false;

		
		bool gpu_ready = false;

		void adjust_property_sizes() {
			target_list.resize(this->rk_graph.latitude.size(), 0);
			// bit vector du nombre de noeuds dont le graphe est constitué
			node_bit_vector.resize(this->node_count, false);
			capacity_coverage_node.resize(this->node_count);
			capacity_coverage_way.resize(this->rk_graph.head.size());
			//way_bit_vector.resize(this->osmwayid_to_idx.size(), false);
			ch_query = RoutingKit::ContractionHierarchyQuery(ch);
			RoutingKit::GeoPositionToNode map_geo_position_(this->rk_graph.latitude, this->rk_graph.longitude);
			this->map_geo_position = map_geo_position_;

			// Initialize GPU with graph topology
			int gpu_result = gpu_init(
				this->rk_graph.head.data(),
				this->tail.data(),
				this->rk_graph.way.data(),
				this->arc_count,
				(unsigned)this->way_osmid.size(),
				this->node_count
			);
			gpu_ready = (gpu_result == 0);
			if (gpu_ready) {
				cout_message("[GPU] GPU acceleration enabled for coverage computation");
			} else {
				cout_message("[GPU] GPU not available, using CPU fallback");
			}
		}

		/**
		 * Load building-to-segment mapping from JSON file.
		 * The JSON format is: {"segment_osm_id": [building_id1, building_id2, ...], ...}
		 */
		void load_building_mapping(std::string mapping_file) {
			long long start_time = RoutingKit::get_micro_time();

			FILE* fp = fopen(mapping_file.c_str(), "r");
			if (!fp) {
				cout_message("Warning: Building mapping file not found: " + mapping_file);
				cout_message("Building coverage will output segment IDs instead of building IDs");
				return;
			}
			...
			if (doc.HasParseError()) {
				cout_message("Warning: Failed to parse building mapping JSON: " + mapping_file);
				return;
			}

			// Parse the mapping: segment_osm_id (string key) -> array of building_ids
			for (auto it = doc.MemberBegin(); it != doc.MemberEnd(); ++it) {
				uint64_t segment_osm_id = std::stoull(it->name.GetString());
				std::vector<uint64_t> building_ids;

				if (it->value.IsArray()) {
					for (auto& building_id : it->value.GetArray()) {
						if (building_id.IsUint64()) {
							building_ids.push_back(building_id.GetUint64());
						} else if (building_id.IsInt64()) {
							building_ids.push_back(static_cast<uint64_t>(building_id.GetInt64()));
						} else if (building_id.IsUint()) {
							building_ids.push_back(building_id.GetUint());
						} else if (building_id.IsInt()) {
							building_ids.push_back(static_cast<uint64_t>(building_id.GetInt()));
						}
					}
				}

				if (!building_ids.empty()) {
					segment_to_buildings[segment_osm_id] = building_ids;
				}
			}

			building_mapping_loaded = true;

			long long end_time = RoutingKit::get_micro_time();
			cout_message("Building mapping loaded: " + std::to_string(segment_to_buildings.size()) +
						" segments with buildings in " + microseconds_to_readable_time_cout(end_time - start_time));
		}

		/**
		 * Get building IDs for a given segment OSM ID.
		 * Returns empty vector if no buildings are linked to this segment.
		 */
		const std::vector<uint64_t>& get_buildings_for_segment(uint64_t segment_osm_id) const {
			static const std::vector<uint64_t> empty_vec;
			auto it = segment_to_buildings.find(segment_osm_id);
			if (it != segment_to_buildings.end()) {
				return it->second;
			}
			return empty_vec;
		}
		...
		void unit_coverage_on_nodes(RoutingKit::BitVector& node_bit_vector, unsigned source, unsigned threshold = 300){
			...
			if (gpu_ready) {
				// GPU-accelerated: threshold distances -> node bit vector
				unsigned nbv_words = (node_count + 63) / 64;
				memset(const_cast<uint64_t*>(node_bit_vector.data()), 0, nbv_words * sizeof(uint64_t));
				gpu_threshold_nodes(distances_to_targets.data(), threshold,
					const_cast<uint64_t*>(node_bit_vector.data()), nbv_words);
			} else {
				// CPU fallback
				for (unsigned i = 0; i < node_count; ++i)
				{
		...
		/**
		 * GPU-accelerated fused coverage: computes both node and way bit vectors in one call.
		 * Replaces the sequential unit_coverage_on_nodes + way marking loop.
		 * Falls back to CPU if GPU is not available.
		 */
		void unit_coverage_gpu_fused(RoutingKit::BitVector& node_bit_vector,
		                             RoutingKit::BitVector& way_bit_vector,
		                             unsigned source, unsigned threshold = 300) {
			...
			if (gpu_ready) {
				unsigned nbv_words = (node_count + 63) / 64;
				unsigned wbv_words = ((unsigned)way_osmid.size() + 63) / 64;
				gpu_fused_coverage(distances_to_targets.data(), threshold,
					const_cast<uint64_t*>(node_bit_vector.data()), nbv_words,
					const_cast<uint64_t*>(way_bit_vector.data()), wbv_words);
			} else {
				// CPU fallback: threshold nodes
				for (unsigned i = 0; i < node_count; ++i) {
					if (distances_to_targets[i] < threshold)
						node_bit_vector.set(i, 1);
				}
				// CPU fallback: mark ways
				for (unsigned i = 0; i < this->rk_graph.head.size(); ++i) {
					if (node_bit_vector.is_set(this->rk_graph.head[i]) && node_bit_vector.is_set(this->tail[i]))
						way_bit_vector.set(this->rk_graph.way[i], 1);
				}
		...
		void capacity_coverage(std::vector<unsigned>& source_list, unsigned threshold = 300){
			...
			unsigned threshold_ms = threshold * 1000;
			...
			cout_message("Start computing the coverage capacity for a " + std::to_string(threshold) + " seconds coverage"
			             + (gpu_ready ? " [GPU]" : " [CPU]"));

			start_time = RoutingKit::get_micro_time();

			if (gpu_ready) {
				// GPU path: run CH queries on CPU, batch all distances, process on GPU
				std::vector<unsigned> all_distances(source_list.size() * node_count);

				for (unsigned s = 0; s < source_list.size(); ++s) {
					std::vector<unsigned> dists = ch_query.reset_source().add_source(source_list[s]).run_to_pinned_targets().get_distances_to_targets();
					memcpy(&all_distances[s * node_count], dists.data(), node_count * sizeof(unsigned));
				}

				gpu_batch_capacity(all_distances.data(), (unsigned)source_list.size(), threshold_ms,
				                   capacity_coverage_node.data(), capacity_coverage_way.data());
			} else {
				// CPU fallback
				for (auto s : source_list) {
					...
					for (unsigned i = 0; i < node_count; ++i) {
						if (distances_to_targets[i] < threshold_ms)
			...
			cout_message(*max_element(capacity_coverage_way.begin(),capacity_coverage_way.end()) + " unité(s) (atteignable sous " + std::to_string(threshold) + " secondes à la limite de vitesse)");
		...
		* @prerequisite capacity_coverage should have been processed first.
	    ...
	    /**
		 * Returns a string of nodes composing the given route:
		 * [[latitude,longitude],[latitude,longitude],...,[latitude,longitude]]
		 *
		 * @param values Container whose values are summed.
		 * @return sum of `values`, or 0.0 if `values` is empty.
		...
		std::string get_route_lat_lon_nodes(float source_lat,float source_lon,float target_lat,float target_lon)
		{
			// Build the index to quickly map latitudes and longitudes
			unsigned source = map_geo_position.find_nearest_neighbor_within_radius(source_lat, source_lon, 1000).id;
			unsigned target = map_geo_position.find_nearest_neighbor_within_radius(target_lat, target_lon, 1000).id;

			if(source == RoutingKit::invalid_id || target == RoutingKit:: invalid_id){
				std::cerr << "Unable to calculate route with source: [" << source_lat << "," << source_lon << "] and target: [" << target_lat << "," << target_lon << "]" << std::endl;
				return "";
			}
			...
			std::string route_polyline = "[";
			int i = 0;
			for(uint64_t x:path) {
				route_polyline += "[" + std::to_string(rk_graph.latitude[x]) + "," + std::to_string(rk_graph.longitude[x]) + "]";
				if(i < path.size() - 1)
					route_polyline += ",";
				i += 1;
			}
